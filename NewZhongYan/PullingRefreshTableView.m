...
- (void)tableViewDidScroll:(UIScrollView *)scrollView
{
    ...
    }else if(_footerView.state == kPRStateLoading){
        return;
...
- (void)refreshLastUpdatedDateWithKey:(NSString*)keyString
{
    NSDateFormatter *formatter = [[NSDateFormatter alloc] init];
    [formatter setAMSymbol:@"上午"];
    [formatter setPMSymbol:@"下午"];
    [formatter setDateFormat:@"MM/dd/yyyy hh:mm:a"];
    _headerView.dateLabel.text = [NSString stringWithFormat:@"上次更新: %@", [formatter stringFromDate:[NSDate date]]];
    [[NSUserDefaults standardUserDefaults] setObject:_headerView.dateLabel.text forKey:keyString];
...
- (void)launchAddRefreshing {
    //if ([APPUtils APPQueue].operationCount) return;
    [self setContentOffset:CGPointMake(0,0) animated:NO];
    [UIView animateWithDuration:kPRAnimationDuration animations:^{
        self.contentOffset = CGPointMake(0, -kPROffsetY-1);
    } completion:^(BOOL bl){
